...
import { usePekerjaan } from "@/lib/hooks/masterData/master-informasi/pekerjaan";

const TableDataPekerjaan = () => {
  const methods = useForm();
  const { Pekerjaan, loading, error } = usePekerjaan();
  const [filteredPatients, setFilteredPatients] = useState(
    Pekerjaan.data || []
  );

  useEffect(() => {
    if (Pekerjaan && Array.isArray(Pekerjaan.data)) {
      setFilteredPatients(Pekerjaan.data);
    }
  }, [Pekerjaan]);
          ...
          <Col sm="12" className="p-3">
            <div className="iq-card p-3">
              <div className="iq-card-header d-flex justify-content-between">
                ...
                <ButtonNav
                  path="/MasterData/master-informasi/master-pekerjaan/add-pekerjaan-form"
                  label="Add Pekerjaan"
                  icon="ri-add-fill"
                  size="sm"
                  variant=""
