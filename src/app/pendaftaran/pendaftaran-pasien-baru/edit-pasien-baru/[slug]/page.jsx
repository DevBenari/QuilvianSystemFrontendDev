...
import {
  AddPasienSlice,
  fetchPendaftaranPasienBaruById,
...
const KioskPendaftaranPasien = memo(({ params }) => {
  const { setValue } = useForm();
  const [dataPasienBaru, setDataPasienBaru] = useState(null);
  ...
  const { selectedPasienBaru } = useSelector((state) => state.pasien);

  // Fetch data Pasien berdasarkan ID

  useEffect(() => {
    dispatch(fetchPendaftaranPasienBaruById(extractIdFromSlug(params.slug)));
  }, [dispatch, params.slug]);

  // Sinkronisasi data Redux dengan State
  useEffect(() => {
    if (selectedPasienBaru) {
      setDataPasienBaru(selectedPasienBaru);
    }
  }, [selectedPasienBaru]);
      ...
      fields: [
        {
          ...
          options: pekerjaanData.map((item) => ({
            label: item.namaPekerjaan,
            value: item.pekerjaanId,
          })),
          colSize: 6,
        },
        {
      ...
      fields: [
        {
          ...
          options: GolonganDarah.map((item) => ({
            label: item.namaGolonganDarah,
            value: item.golonganDarahId,
          })),
          colSize: 6,
        },
        {
  ...
  const formFieldsWithData = formFields.map((section) => ({
    ...
    fields: section.fields.map((field) => ({
      ...field,
      value: dataPasienBaru?.[field.name] ?? "",
    })),
  }));
