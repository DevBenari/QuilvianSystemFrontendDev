...
import ModalInsurance from '@/components/view/kiosk/add-guest-layanan/modal-insurance'; // Make sure path is correct
...
const KioskScreeningCard = ({ onPatientFound, onError }) => {
  ...
  // Add state for holding the found patient data and modal visibility
  ...
  const processScreening = async (data) => {
    ...
    try {
      ...
      } else if (data.noRekamMedis || formData.noRekamMedis) {
        filterParams.noRekamMedis = data.noRekamMedis || formData.noRekamMedis;
      ...
      if (result.payload && result.payload.data && 
        ...
        // Set foundPatient state with the patient data
        const patient = Array.isArray(patientData) && patientData.length > 0 
          ? patientData[0] 
          : patientData;
  ...
  // Function to handle insurance submission
  const handleInsuranceSubmit = (insuranceData) => {
    console.log("Insurance data saved:", insuranceData);
    ...
    <>
      <Card className="shadow-lg border-0 mb-4">
        ...
        <Card.Body className="p-4">
          {foundPatient ? (
            <div className="patient-found-section">
              <Alert variant="success" className="mb-4">
                <div className="d-flex align-items-center">
                  <i className="bi bi-check-circle-fill me-3 fs-3"></i>
                  <div>
                    <h5 className="mb-1">Pasien Ditemukan</h5>
                    <p className="mb-0">ID: {foundPatient.pasienId}</p>
                    <p className="mb-0">Nama: {foundPatient.nama}</p>
                    <p className="mb-0">No. RM: {foundPatient.noRekamMedis}</p>
                  </div>
                </div>
              </Alert>
            ...
            >
              ...
              <Tab eventKey="noRekamMedis" title="Nomor RM">
                <div className="py-3">
      ...
      <ModalInsurance 
        ...
        patientId={foundPatient?.pasienId}
      />
    </>
  );
};
